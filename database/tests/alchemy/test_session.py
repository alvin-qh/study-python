...
def test_insert_and_query() -> None:
    ...
    id_ = session.execute(
        table_core_users.insert().values(
...
def test_delete() -> None:
    ...
    # 返回影响的行数
    rowcount = session.execute(
        table_core_users.delete().where(table_core_users.c.id == id_)
    ).rowcount
...
def test_join() -> None:
    ...
    # 描述组表
    table_core_group = table("core_groups", column("id"), column("name"))

    # 描述用户和组关系表
    table_core_user_group = table(
        "core_user_groups", column("id"), column("user_id"), column("group_id")
    )
    ...
    # 插入组数据
    group_id = session.execute(table_core_group.insert().values(name="VIP")).lastrowid

    # 插入用户组关系数据
    session.execute(
        table_core_user_group.insert().values(user_id=user_id, group_id=group_id)
    ).lastrowid
