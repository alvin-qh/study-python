...
from mongo_graphql.models import (
    Role as RoleModel, Department
)
from tests.mongo_graphql import BaseTest
from .factories import (
    DepartmentFactory,
    EmployeeFactory
)
from .queries import (
    QUERY_DEPARTMENT_BY_NAME, QUERY_EMPLOYEE_BY_NAME, QUERY_EMPLOYEES_BY_DEPARTMENT, CREATE_DEPARTMENT, CREATE_EMPLOYEE
)
...
class TestQueries(BaseTest):
    ...
    def setup_method(self, method):
        ...
        self.employee1 = EmployeeFactory.create(department=self.department1, role=self.role_manager)
        self.employee2 = EmployeeFactory.create(department=self.department1, role=self.role_member)
        self.employee3 = EmployeeFactory.create(department=self.department2, role=self.role_manager)
    ...
    def test_query_department(self):
        result = self.client.execute(
            ...
            variables={"name": self.department1.name}
        )
        assert result == {
            "data": {
                "department": {
    ...
    def test_query_employee(self):
        result = self.client.execute(
            ...
            variables={"name": self.employee2.name}
        )
        assert result == {
            "data": {
                "employee": {
...
class TestMutation(BaseTest):
    def test_department_mutation(self):
        result = self.client.execute(
            CREATE_DEPARTMENT,
            variables={
                "departmentInput": {
    ...
    def test_employee_mutation(self):
        ...
        result = self.client.execute(
            ...
            variables={
                "employeeInput": {
                    "name": "Alvin",
                    "gender": "male",
                    "department": to_global_id(department.__class__.__name__, department.id),
                    "role": "manager"
                }
            }
        )
        ...
        assert employee["department"]["id"] == to_global_id(department.__class__.__name__, department.id)
