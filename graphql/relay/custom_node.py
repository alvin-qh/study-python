...
class CustomNode(Node):
    ...
    def get_node_from_global_id(
        ...
        Returns:
            Node: 实体对象
        """
        type, id = global_id.split(":", 1)
        if only_type:
            assert type == only_type._meta.name

        return info.schema.get_type(type).graphene_type.get_node(info, id)
...
class User(ObjectType):
    ...
    @classmethod
    async def get_node(cls, info: ResolveInfo, id: ID) -> "User":
        """
        根据 `id` 获取当前实体对象
