...
from ..common import render_axis3d, get_axis3d
...
def test_draw_splashes() -> None:
    ...
    通过一组圆心值绘制实心圆圈, 通过 `c` (`color`) 参数指定颜色
    """
    # 绘制坐标系和网格
    render_axis3d(
        (-10, 10),  # x 坐标轴的范围
...
def test_draw_bubbles() -> None:
    """
    绘制泡泡图, 通过 `s` (`size`) 参数指定绘制图形的半径

    通过调色板设置颜色, `c` (`color`) 参数指定 `0~100` 的数值, 通过 `cmap` 参数指定调色板
    """
    N = 50
