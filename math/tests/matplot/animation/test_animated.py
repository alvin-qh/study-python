...
def test_animated_with_fixed_axis() -> None:
    ...
    def calculate_slope(x_: float) -> float:
        ...
        x2 = x_ - 0.05
        y2 = np.sin(x2)
        return (y2 - y1) / (x2 - x1)  # type: ignore
    ...
    def calculate_tangent_line(
        ...
        ys = y_ + k_ * (xs - x_)  # type: ignore
        return xs, ys

    # 计算切线坐标
    k = calculate_slope(x[0])  # 计算斜率
    xs, ys = calculate_tangent_line(x[0], y[0], k)  # type: ignore  # 计算切线
    ...
    def update(n: int) -> None:
        ...
        xs, ys = calculate_tangent_line(x[n], y[n], k)  # type: ignore # 计算切线

        # 更新曲线上的切点坐标
        point.set_data(x[n], y[n])  # type: ignore
        # 更新切线的坐标
        tangent_line.set_data(xs, ys)

        # 更新坐标标识
        x_text.set_text(f"x={x[n]:.3f}")
        y_text.set_text(f"y={y[n]:.3f}")  # type: ignore
