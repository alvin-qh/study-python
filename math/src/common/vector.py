...
from .typedef import Matrix, Number, Polar, Triangle, Vector, Vector2D, Vector3D
...
def perimeter(vs: Sequence[Vector]) -> float:
    """计算由向量围成的周长
...
def linear_combination(
    ...
    Args:
        `vectors` (`Sequence[Vector]`): 向量集合
        `scalars` (`Sequence[Number]`): 每个向量的放大系数

    Returns:
        `Vector`: 组合后的向量
    """
    scaled = [scale(v, s) for v, s in zip(vectors, scalars)]
    return add(*scaled)
