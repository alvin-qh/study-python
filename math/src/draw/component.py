...
def draw2d(
    ...
    Args:
        ...
        `grid` (`Tuple[Number, Number]`, optional): 网格的单位长度. Defaults to `(1, 1)`.
    ...
    for o in objects:
        if isinstance(o, Polygon2D):
            ...
            for i in range(0, len(o.vertices)):
                ...
                plt.plot([x1, x2], [y1, y2], color=o.color.value)
...
class FancyArrow3D(FancyArrowPatch):
    ...
    def do_3d_projection(self, renderer: Any = None) -> Number:
        """绘制 3D 图形"""
