...
class DepartmentQuery(ObjectType):
    ...
    def resolve_department(
        ...
        Returns:
            `Optional[DepartmentModel]`: 查询结果, 表示部门实体对象
        """
        return (
            DepartmentModel.select().where(DepartmentModel.name == name).get_or_none()
        )
...
class EmployeeQuery(ObjectType):
    ...
    def resolve_employee(
        ...
        """
        return EmployeeModel.select().where(EmployeeModel.name == name).get_or_none()
