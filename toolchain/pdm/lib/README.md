...
通过 PDM 可以创建 Python 库项目, 用于编译一个 Python 库, 输出 `.whl` 文件供其它 Python 项目
...
PDM 会在项目中创建 `src` 目录, 所有的项目源代码都应位于 `src` 目录下, 且 `src` 目录下必须具备一个和项目名相同的包 (本例中为 `pdm_lib` 包), 作为当前 Python 库的根包名
...
...
dependencies = [
    "other-pdm-lib @ file:///${PROJECT_ROOT}/../other_lib",
]
```
...
命令执行完毕后, 会在 `dist` 目录中生成 `pdm_lib-0.1.0-py3-none-any.whl` 文件
