...
def test_parametrize_func(test_input, expected):
    """
    参数化:
...
@pytest.mark.parametrize(
    argnames="a, b, result",
    argvalues=[(1, 2, 3), (4, 5, 9)]
)
class TestParamterize:
    """ 参数化也可以应用于测试类, 对类中的所有方法起作用 """

    def test_parametrize_method(self, a, b, result):
        assert a + b == result
...
def test_parametrize_with_skip(a, b, result):
    """
...
def test_parametrize_multi_parameter(a, b):
    """
    多个参数化装饰器, 传入参数组成笛卡尔积
...
def test_parametrize_fixture(parametrize_fixture):
    """
    参数化可以和'fixture'结合使用, 即传入'fixture'作为参数. 参数化的参数值会依次传给'fixture', 实际传入测试函数的参数
    为'fixture'的返回值

    此时'parametrize_fixture_single'作为参数化的名称, 同时也表示名为'parametrize_fixture_single'的'fixture'
...
@pytest.fixture
def parametrize_fixture_multiple(request):
    """
    此时'request.param'为一个字典, 依次为: {"a": 100, "b": 200, "c": 300}, {"a": 1000, "b": 2000, "c": 3000}
    """
...
def test_parametrize6(parametrize_fixture_multiple):
    """
...
@pytest.fixture
def parametrize_fixture_param1(request):
    """
    传递给'request.param'的参数依次为: {"a": 100, "b": 200}, {"a": 1000, "b": 2000}
    """
    a = request.param["a"]
    b = request.param["b"]
    return a + b
...
@pytest.fixture
def parametrize_fixture_param2(request):
    """
...
def test_parametrize7(parametrize_fixture_param1, parametrize_fixture_param2):
    """
