...
可以将一组任务及其参数放入任务队列中, 线程池可以根据池的大小, 批量的并发执行若干任务直到将任务队列中的任务消耗完
...
def test_pool_apply() -> None:
    ...
    因为  `apply` 方法是同步执行的, 必须等到线程函数执行完毕后返回, 如果要并行执行 则可通过 `apply_async` 方法, 返回一个句柄,
    稍后可通过句柄对象的 `get` 方法获取线程执行结果
    """
    # 保存结果的数组
...
def test_pool_map() -> None:
    ...
    相关 `map_sync` 方法可以异步的调用线程池, 即不必等待所有任务执行完毕即可返回一个句柄对象
...
def test_pool_imap() -> None:
    ...
    通过 `pool.imap(func, [a1, a2, a3, a4])` 表示: 依次将参数 `a1`, `a2`, `a3`, `a4` 绑定到 `func` 函数上,
    并从线程池中取一个线程执行. 并返回每次线程执行的结果集合

    通过 `imap` 方法返回一个 `IMapIterator` 类型的迭代器对象, 从迭代器中可以获取每个线程执行的结果

    相关 `imap_unordered` 返回的 `IMapIterator` 迭代器中的执行结果不会严格按照参数顺序, 哪个线程先执行完毕就在迭代器中排在前面
    """
