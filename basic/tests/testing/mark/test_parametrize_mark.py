...
def test_parametrize_func(test_input: str, expected: int) -> None:
    ...
    通过 `mark.parametrize` 可以为测试指定一组参数, 并依次传入测试函数进行测试有几个参数, 就会执行几次测试函数
...
@pytest.mark.parametrize(
    argnames="a, b, result",
    argvalues=[(1, 2, 3), pytest.param(4, 5, 9, marks=pytest.mark.skip)],
)
def test_parametrize_with_skip(a: int, b: int, result: int) -> None:
    """参数化的值可以用 `pytest.param` 产生, 此时可以传入 `marks`

    对这一组参数的测试用例进行标记 `skip`, `skipif`, `xpass`, `xfail` 等标记均可以使用
    """
    assert a + b == result
...
def test_parametrize_fixture(parametrize_fixture: Tuple[int, int]) -> None:
    """参数化可以和 `fixture` 结合使用

    即传入 `fixture` 作为参数. 参数化的参数值会依次传给 `fixture`, 实际传入测试函数的参数为 `fixture` 的返回值
    ...
    """
    assert parametrize_fixture[0] * parametrize_fixture[0] == parametrize_fixture[1]
...
def parametrize_fixture_param1(request: SubRequest) -> int:
    ...
    ```
    """
    a: int = request.param["a"]
    b: int = request.param["b"]
    return a + b
