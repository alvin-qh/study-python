...
def test_async_coroutine() -> None:
    ...
    ```
     ...
     - 以协程方式启动的异步方法必须在一个事件循环中列队等待调用,
     `asyncio.run()` 用于启动一个事件循环, 并执行入口函数
    """
...
def test_coroutine_wait_timeout() -> None:
    ...
    - `asyncio.wait_for(coroutine, timeout)` 函数接受一个 `coroutine` 对象作为参数,
        返回另一个 `coroutine` 对象
    ...
    async def main() -> None:
        ...
        协程入口函数
        """
        # 设定协程执行 1 秒, 启动协程函数并等待 2 秒
        r = await asyncio.wait_for(worker(1), timeout=2)
        # 此时等待成功, 协程执行完毕
        assert r == "OK"

        # 等待失败, 抛出超时异常
        with raises(asyncio.TimeoutError):
            # 设定协程执行 3 秒, 启动协程函数并等待 2 秒
            await asyncio.wait_for(worker(3), timeout=2)
...
def test_task_wait_timeout() -> None:
    ...
    > 注意: 一个任务即便超时, 访问任务的 `done()` 函数也会返回 `True`, 表示任务已完成.
    即超时的任务仍属于已完成任务, 而不是已取消任务
    """
...
def test_coroutine_cancel() -> None:
    ...
    async def main() -> None:
        ...
        """
        # 包装异步函数为 Future 对象
        future = asyncio.shield(worker(1))
        # 取消异步调用
        future.cancel()
...
def test_coroutine_exception() -> None:
    ...
    async def main() -> None:
        ...
        with raises(IOError):
            # 等待一个会抛出异常的协程执行
            await worker(exception=IOError)

        # 产生一个 Future 对象, 包装一个会抛出异常的协程执行
        future = asyncio.shield(worker(exception=IOError))
...
def test_task_exception() -> None:
    ...
    async def main() -> None:
        ...
        task = asyncio.create_task(worker(1, exception=IOError))
...
def test_multiple_coroutines() -> None:
    ...
    - 可以通过 `asyncio.gather(...coroutines)` 函数一次性批量执行多个异步调用,
    得到一个 `_GatheringFuture` 对象, 表示该组调用的存根
    ...
    async def main() -> None:
        ...
        # 整组协程执行时间为 3 秒以上, 即组内最长协程执行时间的协程
        assert timeit.default_timer() - start >= 3
...
def test_multiple_tasks() -> None:
    ...
    async def main() -> None:
        ...
        tasks = asyncio.gather(
            asyncio.create_task(worker(1, id_=3)),
            asyncio.create_task(worker(2, id_=2)),
            asyncio.create_task(worker(3, id_=1)),
        )
        ...
        # 整组协程执行时间为 3 秒以上, 即组内最长协程任务执行时间的协程
        assert timeit.default_timer() - start >= 3
...
def test_async_callback() -> None:
    ...
    callbacks: List[Tuple[str, str, asyncio.AbstractEventLoop]] = []
    ...
    async def main() -> None:
        ...
        # 将协程对象包装为 Future 对象
        future = asyncio.ensure_future(worker(1, id_=1))
        # 设置任务名称
        ...
        assert callbacks[0][2] == asyncio.get_running_loop()

        # 对 Task 执行后进行回调

        # 将协程对象包装为 Task 对象
        task = asyncio.create_task(worker(1, id_=2))
        # 设置任务名称
        ...
        assert callbacks[1][2] == asyncio.get_running_loop()
...
def test_event_loop() -> None:
    ...
    async def main() -> None:
        ...
        # 启动协程
        future = asyncio.ensure_future(worker(1))
        # 为协程任务设置名称
        future.set_name("Task-1")
        # 为协程结束设置回调函数
        future.add_done_callback(callback)
...
def test_none_async_call_in_event_loop() -> None:
    ...
    - 另外, `event_loop.call_soon` 函数并不能让指定的调用立即执行, 因为此时当前线程还在执行代码,
    事件循环得不到执行机会, 只有当前线程执行完指定代码后, 才有机会调度事件循环从事件队列中执行一个函数
    """
    ...
    async def is_finished() -> bool:
        ...
        """
        # 判断字典是否包含了 5 项内容
        if len(called_tasks) < 5:
            # 数量不足, 当前协程等待 1 秒后返回 False
    ...
    async def main() -> None:
        ...
        # 设置 2.1 秒钟后执行 id=3, 由于 2 秒后 id=4 会执行, 耗时 1 秒, 所以 id=3 会在 id=4 后执行
        # 实际在 3 秒钟后执行
        loop.call_later(2.1, sync_func, 3)
        # 设置 2 秒钟后执行 id=4, 由于 id=1 和 id=2 一共会执行 2 秒钟, 所以 id=4 会在 id=2 之后被执行
        loop.call_later(2, sync_func, 4)

        # 获取协程执行的当前时间
        now = loop.time()

        # 指定在协程当前时间后第 3 秒执行 id=5, 但由于之前的操作已经耗费了 4 秒, 所以该函数只能在第 4 秒时被执行
        # 如果将参数改为 now + 2, 则 id=5 会先于 id=3 被执行（即在 id=4 之后）
    ...
    # id=1 立即执行
    assert int(called_tasks[1]) == 0
    # id=2 于 1 秒后执行 (在 id=1 执行完毕后执行)
    assert int(called_tasks[2]) == 1
    # id=3 于 3 秒后执行 (在 id=4 执行完毕后执行)
    assert int(called_tasks[3]) == 3
    # id=4 于 2 秒后执行 (在 id=2 执行完毕后执行)
    assert int(called_tasks[4]) == 2
    # id=5 于 4 秒后执行 (在 id=3 执行完毕后执行)
    assert int(called_tasks[5]) == 4
...
def test_async_lock() -> None:
    ...
    async def main() -> None:
        ...
        """
        # 组合两个协程函数同时执行
        r = await asyncio.gather(locked_job(1), locked_job(2))
        assert len(r) == 2

        # 第一个结果为 ID=1 函数返回
        assert r[0]["id"] == 1
        assert r[0]["finish_time"] == 1  # 整体执行 1 秒
...
def test_async_event() -> None:
    ...
    async def main() -> None:
        ...
        """
        # 同时启动两个协程函数
        group = asyncio.gather(event_job(1), event_job(2))
