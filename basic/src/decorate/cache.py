...
def _check_duplicated_cache_key(key: str) -> None:
    ...
    如果 Key 中包含形如 `{x}`, `{y}` 这类的占位符, 则统一替换为 `{__exp__}` 后进行查找. 也就是说 `demo_{x}_{y}` 和 `demo_{a}_{b}`
    被认为是重复的字符串
    ...
    if canonical_key in _cache_keys:
        raise KeyError(f"Duplicate cache key: {key}, existing key: {key}")
...
def memo(key: str) -> Any:
    ...
    缓存操作适用于幂等性函数 (即参数相同, 则返回值一定相同的函数). 如果函数结果被缓存, 则从缓存中获取对应值
