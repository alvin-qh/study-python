...
class AIOTicker:
    ...
    Python 具备 `__iter__` 和 `__next__` 两个魔法方法, 前者用于返回一个迭代器对象, 后者用于返回每次迭代的结果. 具备 `__next__`
    方法的类型可被看作是一个迭代器类型, 具备 `__iter__` 方法的类型可以返回一个迭代器对象, 可被用于 `for ... in ...` 形式的循环中.
...
async def async_ticker(delay: float, to: int) -> AsyncGenerator[int, None]:
    ...
    返回一个 `AsyncGenerator` 类型的对象, 表示异步生成器对象
