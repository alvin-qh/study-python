...
def test_static_import() -> None:
    ...
    """
    # 静态导入所需的标识
    from .sub import Class, function, value
...
def test_dynamic_import() -> None:
    ...
    """
    # 使用绝对层级导入
    m = import_module("module_.sub.plugin1")
    assert m.__name__ == "module_.sub.plugin1"

    # 使用相对层级导入
    # __package__ 表示相对于当前包
    m = import_module(".sub.plugin1", package=__package__)
    ...
    # 导入另一个包
    m = import_module(".sub.plugin2", package=__package__)
    plugin = m.Plugin()
...
def test_builtin_import() -> None:
    ...
    m = __import__("module_.sub", fromlist=["plugin1", "plugin2"])
    assert m.__name__ == "module_.sub"
